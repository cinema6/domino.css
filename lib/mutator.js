...
var getID = require('./get_id');

function createNode(config) {
    var element = document.createElement(config.tagName);
    var length = config.attributes.length;

    while (length--) {
        element.setAttribute(config.attributes[length].name, config.attributes[length].value);
    }
...
function mutateDOM(contexts) {
    var length = contexts.length;
    var index, context, parent, childrenLength, oldOrder, newOrder, moves, movesLength, move;
    ...
    for (index = 0; index < length; index++) {
        ...
        while (movesLength--) {
            ...
            parent.insertBefore(move.element, newOrder[move.index + 1] || null);
        }
    }
}
    ...
    function apply(rules) {
        ...
        for (index = 0; index < length; index++) {
            ...
            for (nodeIndex = 0; nodeIndex < nodesLength; nodeIndex++) {
                ...
                for (configIndex = 0; configIndex < configsLength; configIndex++) {
                    config = configs[configIndex];

                    addNode(node, createNode(config), Infinity, contexts);
                }
            }
        }
